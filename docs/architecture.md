...
Twenty-seven tables. Some deliberate compressions to avoid a sprawl of near-identical ones:
...
An operator knows the employer they want on campus by name. Everything
downstream — `AtsBoardProbe`, `SourceDiscoveryService`, the policy gate, the
ingestion pipeline — already worked from a domain. `CompanyResolver` is the step
that was missing, and it is the only new service in this area.

Resolution is deterministic and ordered by how much a source of belief is worth:
a domain already recorded against the company, then the built-in market seed
list, then domains constructed from the name and confirmed by the site itself
answering with that name. No search API and no model, because a wrong domain is
not a cosmetic error — it puts another company's jobs in front of students under
an employer's name. When the evidence does not settle it, the honest answers are
`AMBIGUOUS` and `NOT_FOUND`, and a person supplies the domain.

**Resolving and discovering are two calls, on purpose.** A resolve returns
candidates and registers nothing; the operator confirms one domain per company;
only then is a board probed and a source registered. The confirmation authorises
an outbound request to somebody else's servers, so a person stands between the
guess and the consequence. Nothing is pre-selected in the UI, including a single
confidently resolved domain — a pre-ticked box is a default, not a decision.

A source discovered this way is registered through the unchanged
`SourceDiscoveryService`, lands at `DISCOVERED`, and passes the same robots,
terms and access review as a hand-submitted one. Arriving from a company name
earns a source nothing.

### Outbound request safety

Every URL CareerFlux fetches starts as something an operator typed, so `SafeUrlValidator`
decides whether it may be fetched at all. It resolves the host and checks **every**
address the name answers with — hostname strings are not a control, since
`127.0.0.1`, `2130706433`, `[::ffff:127.0.0.1]` and any attacker-controlled name
with an A record all reach the same place. Loopback, link-local, RFC1918,
unique-local, carrier-grade NAT, reserved, multicast and the cloud metadata
endpoints are refused; so is anything that is not `https` on 443. A name that
answers with a mix of public and private addresses is refused outright, because a
mixed answer has no safe reading.

`SafeClientHttpRequestFactory` applies this in the transport rather than at each
call site, so a new adapter cannot forget it, and it disables the redirect
following that `HttpURLConnection` does inside the connection — a 3xx followed
invisibly turns one validated request into an unvalidated one. Redirects are
taken explicitly instead, one validated hop at a time, capped at three.
Careers-page and home-page reads are size-capped so a hostile host cannot stream
indefinitely into a discovery run.

The residual gap is stated rather than papered over: between validation and the
socket connecting, a hostile DNS server can change its answer. Closing that
entirely means connecting to a pinned address instead of a name, which the JDK's
`HttpURLConnection` does not allow without replacing the transport.

### Placement workflow

A shortlist was a yes-or-no fact: a student is on it or they are not. That is the
right shape for the decision it records and it is not enough to run a drive with,
because between "shortlisted" and "placed" a college invites students, waits, and
decides — and none of that had anywhere to live.

Six stages: `SHORTLISTED → INVITED → INTERESTED → SELECTED`, with `NOT_PROCEEDING`
and `DECLINED` as the two ways it can end early. The stage is a column on the
existing shortlist row rather than a new table, because the thing moving through
the workflow is exactly the thing that row already identifies; a parallel
`placements` table keyed on the same pair would be the same row twice, with two
chances to disagree about who is in a drive.

Three properties are load-bearing.

**The actor is part of the rule, not just the record.** `PlacementStage.canMoveTo`
takes an `ActorKind`, and the same move is legal for one kind and refused for the
other. Staff can invite and select; only the student can say `INTERESTED` or
`DECLINED`. If a placement officer could mark a student interested, the invitation
step would become paperwork and the record would stop meaning what it says.

**`NOT_PROCEEDING` and `DECLINED` are separate stages** because afterwards that is
the only thing anybody asks: did the college turn the student down, or did the
student turn the college down. Collapsing them into one "closed" state would lose
the fact and cannot be reconstructed.

**History is transactional; audit is not.** `placement_stage_changes` is
append-only and written in the same transaction as the stage it describes, so the
two cannot disagree. `audit_events` is deliberately `REQUIRES_NEW` — it commits on
its own, which is what makes it a reliable cross-cutting trail and exactly what
disqualifies it from being this record: it would keep a line about a transition
that later rolled back. Both are kept, for different questions.

Concurrency is handled by `@Version` on the shortlist row, the same mechanism
`Job` already uses, so two people acting on one candidate at the same moment
produce a conflict rather than a lost decision.
